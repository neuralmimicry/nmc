...
Derive NMC release, build, and tag metadata from VERSION, git, and optional
environment overrides.

Options:
  --build-version     Print only the runtime build version.
  --release-version   Print only the VERSION file release version.
...
Environment overrides:
  NMC_VERSION, NMC_VERSION_MAJOR, NMC_VERSION_MINOR,
  NMC_BUILD_NUMBER, BUILD_NUMBER, NMC_GIT_COMMIT, GIT_COMMIT.
USAGE
}
...
[[ -n "$release_version" ]] || die "could not read release version from VERSION"

read -r -a release_parts <<<"$(version_numbers "$release_version")"
major="${release_parts[0]:-0}"
minor="${release_parts[1]:-0}"

major_override=$(env_int NMC_VERSION_MAJOR || true)
minor_override=$(env_int NMC_VERSION_MINOR || true)
...
explicit_version=$(env_first NMC_VERSION || true)
build_number=$(env_int NMC_BUILD_NUMBER BUILD_NUMBER || true)
...
git_commit=$(env_first GIT_COMMIT NMC_GIT_COMMIT || true)
...
case "$OUTPUT" in
  ...
  github-output)
    ...
    printf 'NMC build version: %s\n' "$build_version"
    printf 'NMC release tag: %s\n' "$tag"
    ;;
  env)
    printf 'NMC_RELEASE_VERSION=%s\n' "$release_version"
    printf 'NMC_BUILD_VERSION=%s\n' "$build_version"
    printf 'NMC_BUILD_NUMBER=%s\n' "$build_number"
    printf 'NMC_GIT_COMMIT=%s\n' "$git_commit"
    printf 'NMC_GIT_COMMIT_SHORT=%s\n' "$commit_short"
    printf 'NMC_VERSION_SOURCE=%s\n' "$version_source"
    printf 'NMC_TAG=%s\n' "$tag"
    ;;
  *)
