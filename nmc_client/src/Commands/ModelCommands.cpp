...
// --- ModelCommand (Parent) ---
ModelCommand::ModelCommand() : BaseCommand("model", "Manage models in NMC") {
...
// --- ModelUploadCommand ---
ModelUploadCommand::ModelUploadCommand() : BaseCommand("upload", "Uploads a new Model Version") {
...
int ModelUploadCommand::execute(const std::map<std::string, std::string>& parsedFlags, const std::vector<std::string>& parsedArgs, const CLI::GlobalFlags& globalFlags) {
    ...
    Models::CloudResponse response = apiClient.uploadModel(filePath, sku, registryName, tag);
